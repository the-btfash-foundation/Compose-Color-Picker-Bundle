 ...
 */
@Composable
internal fun CompositeSliderPanel(
    ...
    val inputColor = remember(inputColorModel, compositeColor) {
        compositeColor.convertTo(inputColorModel)
    }
    ...
    when (inputColor) {
        is ColorHSL -> {
            CompositeSliderPanelHSL(
                ...
                onColorChange = { onColorChange(it.convertTo(outputColorModel)) },
        ...
        is ColorRGB -> {
            SliderDisplayPanelRGBA(
                color = inputColor,
                modifier = modifier,
                onChange = { onColorChange(it.convertTo(outputColorModel)) },
                showAlphaSlider = showAlphaSlider
            )
        }
    }
...
@Composable
private fun CompositeSliderPanelHSL(
    modifier: Modifier,
...
@Composable
private fun CompositeSliderPanelHSV(
    modifier: Modifier,
